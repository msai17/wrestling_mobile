import 'package:wrestling_hub/core/usecase.dart';
import 'package:wrestling_hub/src/data/video/models/video.dart';
import 'package:wrestling_hub/src/domain/video/repository/video_repository.dart';

class IsFavoriteVideoUseCase implements UseCase<bool, Video>{


  IsFavoriteVideoUseCase(this._videoRepository);

  final VideoRepository _videoRepository;

  @override
  Future<bool> call({void params}) {
    return _videoRepository.isFavorite(params as Video);
  }
}